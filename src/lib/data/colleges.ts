...
export async function getColleges(filters: CollegeFilters = {}) {
  try {
    ...
    if (filters.search) query = query.ilike("college_name", `%${filters.search}%`);
    if (filters.location) query = query.eq("city", filters.location);
    if (filters.maxFees) query = query.lte("annual_fees", filters.maxFees);
    if (filters.course) query = query.ilike("courses_offered", `%${filters.course}%`);

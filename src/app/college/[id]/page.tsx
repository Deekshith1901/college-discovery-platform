...
export default async function CollegeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const formattedFees =
    Number.isFinite(feesValue) && feesValue > 0 ? feesValue.toLocaleString() : "Not available";
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-zinc-700">Fees: {formattedFees === "Not available" ? formattedFees : `INR ${formattedFees}`}</p>
        <div className="mt-4">
          ...
          <CardContent>
            <ul className="list-disc space-y-1 pl-5 text-sm text-zinc-700">
              {courses.length ? (
                ...
                <li className="list-none text-zinc-500">Courses data not available.</li>
              )}
            </ul>
          </CardContent>
        </Card>
            ...
            {reviews.length ? (
              reviews.map((review, idx) => (
                <p key={`${review.user}-${idx}`} className="text-sm text-zinc-700">
                  <span className="font-medium">{review.user}</span> ({review.rating}/5):{" "}
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-6">
        ...
        <p className="mt-2 text-zinc-700">{data.overview || "Overview not available."}</p>
      </section>
    </div>
  );
}
